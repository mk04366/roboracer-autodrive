...
def evaluate_samples_vectorized(state0, traj, U_samples, dt, L, delta_max, a_max, v_max, weights):
    ...
    # Stage cost (per-sample loop retained for clarity & path projection)
    for i in range(S):
        cost = 0.0
        for k in range(N):
            ...
            e_l = float(np.dot(dx, tangent))      # along-track error (progress deviation)
            e_c = float(np.dot(dx, normal))       # cross-track error
            e_psi = wrap_angle(thk - theta_r)     # heading error
            kappa = curvature_at(traj, idx)
            v_ref = v_max / (1.0 + weights['kappa_gain'] * kappa)
            u = U_samples[i,k]
            delta = u[0] * delta_max
            delta_ff = math.atan(max(-0.99, min(0.99, L * kappa)))  # feedforward steering angle
            cost += weights['wc'] * (e_c**2) + weights['wl'] * (e_l**2)
            cost += weights['wpsi'] * (e_psi**2)
            cost += weights['wv'] * ((vk - v_ref)**2)
            # split control weights
            cost += weights['w_steer'] * (u[0]**2) + weights['w_throttle'] * (u[1]**2)
            cost += weights['wdelta_ff'] * ((delta - delta_ff)**2)
            if k > 0:
        ...
        e_psi_f = wrap_angle(thf - theta_r)
        cost += weights['terminal'] * (e_c**2 + e_l**2 + e_psi_f**2)
        costs[i] = cost
    return costs, states

# New helper to compute feedforward & error-based steering bias
def steering_bias(state, traj, L, delta_max, weights):
    x, y, th, v = state
    if traj.shape[0] < 3:
        return 0.0
    idx, t, proj, s = nearest_point_on_path(traj, x, y)
    kappa = curvature_at(traj, idx)
    theta_r = path_tangent(traj, idx)
    e_psi = wrap_angle(th - theta_r)
    tangent = np.array([math.cos(theta_r), math.sin(theta_r)])
    normal = np.array([-math.sin(theta_r), math.cos(theta_r)])
    dx = np.array([x, y]) - proj
    e_c = float(np.dot(dx, normal))
    delta_ff = math.atan(L * kappa)  # unconstrained
    # simple error feedback gains (can expose as params later)
    k_e = weights.get('k_e', 0.4)
    k_psi = weights.get('k_psi', 0.8)
    delta_des = delta_ff + k_e * e_c + k_psi * e_psi
    return float(np.clip(delta_des / delta_max, -1.0, 1.0))

def vectorized_random_shooting(state0, traj, N, dt, L, delta_max, a_max, v_max, num_samples, prev_U, weights):
    S = int(num_samples)
    steer_bias_val = steering_bias(state0, traj, L, delta_max, weights)
    sigma = weights.get('sample_sigma', 0.25)
    if prev_U is None:
        U_samples = np.zeros((S, N, 2))
        # Half uniform broad, half biased Gaussians around steering bias
        half = S // 2
        U_samples[:half, :, 0] = np.random.uniform(-1.0, 1.0, size=(half, N))
        U_samples[:half, :, 1] = np.random.uniform(0.0, 1.0, size=(half, N))
        U_samples[half:, :, 0] = np.clip(steer_bias_val + sigma * np.random.randn(S - half, N), -1.0, 1.0)
        U_samples[half:, :, 1] = np.clip(0.6 + 0.2 * np.random.randn(S - half, N), 0.0, 1.0)
    else:
        base = np.vstack((prev_U[1:], np.zeros((1,2))))
        base[:,1] = np.clip(base[:,1], 0.0, 1.0)
        U_samples = base[None,:,:] + sigma * np.random.randn(S, N, 2)
        # Inject some biased steering rows
        inject = min(4, S)
        U_samples[:inject,:,0] = np.clip(steer_bias_val + sigma * np.random.randn(inject, N), -1.0, 1.0)
    ...
    return U_samples[best_idx], states[best_idx], float(costs[best_idx])
...
class MPCCNode(Node):
    def __init__(self):
        ...
        # New: weight parameters (can override via ROS params)
        self.declare_parameter('w_wc', 80.0)
        self.declare_parameter('w_wl', 1.0)
        self.declare_parameter('w_wpsi', 10.0)
        self.declare_parameter('w_wv', 1.0)
        self.declare_parameter('w_wu', 0.5)
        self.declare_parameter('w_wdu', 5.0)
        self.declare_parameter('w_wdelta_ff', 2.0)
        self.declare_parameter('w_terminal', 80.0)
        self.declare_parameter('kappa_gain', 5.0)
        self.declare_parameter('log_every', 10)
        self.declare_parameter('w_wsteer', 0.2)
        self.declare_parameter('w_wthrottle', 0.1)
        self.declare_parameter('sample_sigma', 0.35)
        self.declare_parameter('k_e', 0.4)
        ...
        self.weights = {
            'wc': float(self.get_parameter('w_wc').get_parameter_value().double_value),
            'wl': float(self.get_parameter('w_wl').get_parameter_value().double_value),
            'wpsi': float(self.get_parameter('w_wpsi').get_parameter_value().double_value),
            'wv': float(self.get_parameter('w_wv').get_parameter_value().double_value),
            'w_steer': float(self.get_parameter('w_wsteer').get_parameter_value().double_value),
            'w_throttle': float(self.get_parameter('w_wthrottle').get_parameter_value().double_value),
            'wdu': float(self.get_parameter('w_wdu').get_parameter_value().double_value),
            'wdelta_ff': float(self.get_parameter('w_wdelta_ff').get_parameter_value().double_value),
            'kappa_gain': float(self.get_parameter('kappa_gain').get_parameter_value().double_value),
            'terminal': float(self.get_parameter('w_terminal').get_parameter_value().double_value),
            'sample_sigma': float(self.get_parameter('sample_sigma').get_parameter_value().double_value),
            'k_e': float(self.get_parameter('k_e').get_parameter_value().double_value),
            'k_psi': float(self.get_parameter('k_psi').get_parameter_value().double_value)
        }
        ...
        self.control_rate = 1.0 / 0.02
        self.timer = self.create_timer(0.02, self.control_loop)
        self.warned_no_pose = False
        self.get_logger().info('MPCC controller (vectorized) started with heading & curvature feedforward costs')
    ...
    def control_loop(self):
        ...
        warm = np.vstack((self.prev_U[1:], np.zeros((1,2)))) if self.prev_U is not None else None
        start = time.time()
        ...
        self.iter_count += 1
        if self.iter_count % max(1, self.log_every) == 0:
            self.get_logger().info(f"[MPCC] t_solve={elapsed_ms:.1f} ms S={self.num_samples} N={self.N} cost={best_cost:.2f}")
        if best_U is None:
