...
class AutodriveEnv(gym.Env):
    ...
    # === DYNAMIC SPEED REWARD ===
        ...
        # Max Speed (Straights) = 4.0 m/s
        # Min Speed (Heavy Turns) = 2.2 m/s (increased to prevent near-stopping)
        # Scaling: As kappa increases, speed drops linearly (gentler slope).
        MAX_STRAIGHT_SPEED = 4.0
        MIN_CURVE_SPEED = 2.2  # Higher minimum to keep car moving through curves
        KAPPA_SCALING_FACTOR = 2.0  # Reduced from 3.0 to slow down less in curves
        
        # Formula: 4.0 - (2.0 * k). If k=0.6 -> 4.0 - 1.2 = 2.8 m/s.
        synthetic_target = MAX_STRAIGHT_SPEED - (KAPPA_SCALING_FACTOR * max_future_kappa)
        
        # Clamp to ensure we maintain good speed even in tight curves
        dynamic_target_speed = np.clip(synthetic_target, MIN_CURVE_SPEED, MAX_STRAIGHT_SPEED)
        ...
        # 2. Underspeed Penalty - Prevent going too slow in curves
        underspeed_penalty = 0.0
        if self.current_speed < MIN_CURVE_SPEED:
            # Penalty increases as speed drops below minimum
            underspeed_violation = MIN_CURVE_SPEED - self.current_speed
            underspeed_penalty = -4.0 * underspeed_violation  # Penalize slow driving
            
            if underspeed_penalty < -2.0:
                self.node.get_logger().debug(
                    f"UNDERSPEED! Min={MIN_CURVE_SPEED:.2f}, Cur={self.current_speed:.2f}, Pen={underspeed_penalty:.2f}"
                )
        
        # 3. Curvature Overspeed Penalty (Negative Reward)
        # Less strict: give more buffer for curve speed adaptation
        overspeed_penalty = 0.0
        
        # Wider buffer (0.8) for smoother curve handling
        speed_limit = dynamic_target_speed + 0.8
        
        if self.current_speed > speed_limit:
            # Penalty proportional to violation (reduced from -5.0)
            violation = self.current_speed - speed_limit
            overspeed_penalty = -3.0 * violation
            
            # Massive penalty for gross speeding (> 1.5 m/s over limit)
            if violation > 1.5:
                 overspeed_penalty -= 8.0
        ...
        if min_dist < COLLISION_THRESHOLD:
            collision_penalty = -30.0  # Harsh penalty for collision
        else:
            collision_penalty = 0.0
            
        # Combine Rewards
        total_reward = speed_reward + underspeed_penalty + overspeed_penalty - smoothness_penalty + traj_tracking_reward + collision_penalty
        
        # Debug logging
        self.node.get_logger().debug(
            f"Reward: speed={speed_reward:.2f}, underspeed={underspeed_penalty:.2f}, overspeed={overspeed_penalty:.2f}, "
            f"smooth={smoothness_penalty:.2f}, traj={traj_tracking_reward:.2f}, crash={collision_penalty:.2f}, total={total_reward:.2f}"
        )
